...
// Define a type for rental detailscheckout
...
const CheckoutPage = () => {
  ...
  const calculateRentalDetails = useCallback((): RentalDetailsType | null => {
  ...
  useEffect(() => {
    setRentalDetails(calculateRentalDetails());
  ...
  const { mutate, isPending: isPendingNewRequest } = useMutation({
    mutationFn: async () =>
      await request.post(`/rental-requests`, {
        carId: params.id,
        startDate: startDate,
        endDate: endDate,
      }),
